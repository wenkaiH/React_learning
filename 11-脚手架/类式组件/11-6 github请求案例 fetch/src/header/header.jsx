...
export default class Header extends Component {
     ...
     search = async() =>{
        ...
        try {
            //链接网络,获取成功的返回值
            const res = await fetch(`http://localhost:3000/api1/search/users?q=${keyWord}`)
            //通过成功的返回值，获取数据 json()只是获取json文件,还有其他获取的方法，但是这些方法只能使用一个
            const users = await res.json();
           //const wang2 = await wang.clone(); 将返回的成功结果复制一份,提取出数据中想要的结果
